...
int test1() {
    ...
    for (int i = 0; i < total ; ++i) {
        ...
        while (not done) {
            done = mh_moves::split(original_part_ptr, split_part_ptr, split_prob);
        }
...
int test_pdr2(const int total, const random_gen_paras& paras) {
    ...
    test_t tests;
    BOOST_FOREACH (const std::string& msg,  move_pdr_test(original_part_ptr, "Merge", total, tests)) {
        diag("%s", msg.c_str());
    }
    BOOST_FOREACH (const test_t::value_type& item, tests) {
        ok(item.second, "%s", item.first.data());
    }
    return 0;
}
