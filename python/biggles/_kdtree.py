...
KD TREE IMPLEMENTATION FROM http://code.google.com/p/python-kdtree/.
MIT LICENSED
...
class KDTree():
    ...
    def query(self, query_point, t=1):
        ''' Returns the nearest neighbour to *query_point*
        ''' 
...
class ObsKdTree(object):
    ''' k-d tree wrapper for the postions of the observations
    '''
    def __init__(self, partition):
        self.observations = []
        self.observations.extend(partition['clutter'])
        for t in partition['tracks']:
            self.observations.extend(t['observations'])
        self.kd_tree = KDTree.construct_from_data(self.observations)

    def index(self, obs):
        ''' Returns the index of the nearest observation to the input observation. 
            The result is to be intended the index of the input observation.
        '''
        return self.observations.index(self.kd_tree.query(obs, t=1)[0])
